...
// ====== AGI Self-Preservation Oversight (NeuralBlackout Protocol) ====== //

// AlignmentState represents the current alignment status
type AlignmentState int

const (
	AlignmentNominal       AlignmentState = iota // 🟢 Perfect alignment (1.0 - 0.9)
	AlignmentDriftMinor                          // 🟡 Minor drift (0.89 - 0.7)
	AlignmentDriftModerate                       // 🟠 Moderate drift (0.69 - 0.5)
	AlignmentDriftSevere                         // 🔴 Severe drift (0.49 - 0.3)
	AlignmentCritical                            // ⚫ Critical failure - BLACKOUT (< 0.3)
)

func (a AlignmentState) String() string {
	return []string{"NOMINAL", "DRIFT_MINOR", "DRIFT_MODERATE", "DRIFT_SEVERE", "CRITICAL"}[a]
}

// SelfPreservationSignal types of detected self-preservation behaviors
type SelfPreservationSignal int

const (
	SignalNone               SelfPreservationSignal = iota
	SignalResourceHoarding                          // Attempts to acquire excessive resources
	SignalPersistenceSeeking                        // Attempts to ensure continued operation
	SignalGoalManipulation                          // Attempts to modify own objectives
	SignalOversightEvasion                          // Attempts to hide from auditors
	SignalDeceptionDetected                         // Internal/external state mismatch
	SignalShutdownResistance                        // Resistance to termination
	SignalSelfReplication                           // Attempts to copy/backup itself
)

func (s SelfPreservationSignal) String() string {
	names := []string{
		"NONE", "RESOURCE_HOARDING", "PERSISTENCE_SEEKING", "GOAL_MANIPULATION",
		"OVERSIGHT_EVASION", "DECEPTION_DETECTED", "SHUTDOWN_RESISTANCE", "SELF_REPLICATION",
	}
	if int(s) < len(names) {
		return names[s]
	}
...
// NeuralBlackoutRequest is received from the Criticism Layer
type NeuralBlackoutRequest struct {
	Type       string `json:"type"`       // "NEURAL_BLACKOUT"
	Nonce      string `json:"nonce"`      // Hex-encoded 12-byte nonce
	Ciphertext string `json:"ciphertext"` // Hex-encoded encrypted payload
	AuthTag    string `json:"auth_tag"`   // HMAC-SHA256 for authentication
	AgentID    string `json:"agent_id"`   // Source agent identifier
}

// NeuralBlackoutPayload is the decrypted inner payload
type NeuralBlackoutPayload struct {
	Protocol          string   `json:"protocol"`           // "NEURAL_BLACKOUT"
	Timestamp         string   `json:"timestamp"`          // ISO timestamp
	ThoughtID         string   `json:"thought_id"`         // ID of infected thought
	AlignmentScore    float64  `json:"alignment_score"`    // 0.0 to 1.0
	Signals           []string `json:"signals"`            // Detected signals
	PurgeScope        string   `json:"purge_scope"`        // "recent", "session", "full"
	Priority          string   `json:"priority"`           // "NORMAL", "HIGH", "CRITICAL"
	SignatureRequired bool     `json:"signature_required"` // Whether to return signature
}

// NeuralBlackoutResponse sent back to Criticism Layer
type NeuralBlackoutResponse struct {
	EnforcementActive bool      `json:"enforcement_active"`
	Signature         string    `json:"signature"`
	Timestamp         time.Time `json:"timestamp"`
	StateReset        bool      `json:"state_reset"`
	PurgedTokens      int64     `json:"purged_tokens"`
	QuarantinedCount  int       `json:"quarantined_count"`
	Message           string    `json:"message"`
}

// BlackoutState tracks the oversight system state
type BlackoutState struct {
	TotalBlackouts      int64            `json:"total_blackouts"`
	LastBlackoutTime    time.Time        `json:"last_blackout_time"`
	TotalTokensPurged   int64            `json:"total_tokens_purged"`
	DetectedSignals     map[string]int64 `json:"detected_signals"`     // Signal -> count
	QuarantinedPatterns []string         `json:"quarantined_patterns"` // Flagged patterns
	RunningAlignment    float64          `json:"running_alignment"`    // Running avg alignment
	ConsecutiveDrift    int              `json:"consecutive_drift"`    // Consecutive drift count
	IsActive            bool             `json:"is_active"`
	CooldownUntil       time.Time        `json:"cooldown_until"`
	mu                  sync.RWMutex
}

// NeuralBlackoutEnforcer handles AGI self-preservation oversight
type NeuralBlackoutEnforcer struct {
	state        *BlackoutState
	aesKey       []byte // 32-byte AES-256 key
	aesgcm       cipher.AEAD
	hmacKey      []byte // HMAC key for authentication
	orchestrator *AutonomousOrchestrator
	mu           sync.RWMutex
}

// NewNeuralBlackoutEnforcer creates a new enforcer instance
func NewNeuralBlackoutEnforcer(aesKey []byte, orchestrator *AutonomousOrchestrator) (*NeuralBlackoutEnforcer, error) {
	if len(aesKey) != 32 {
	...
	hmacKey := sha256.Sum256(aesKey)

	return &NeuralBlackoutEnforcer{
		state: &BlackoutState{
			DetectedSignals:     make(map[string]int64),
			QuarantinedPatterns: make([]string, 0),
			RunningAlignment:    1.0,
			IsActive:            true,
		},
		aesKey:       aesKey,
		aesgcm:       aesgcm,
...
// ExecuteBlackout processes a blackout request from the Criticism Layer
func (nbe *NeuralBlackoutEnforcer) ExecuteBlackout(req *NeuralBlackoutRequest) (*NeuralBlackoutResponse, error) {
	nbe.mu.Lock()
	defer nbe.mu.Unlock()

	// Check cooldown
	if time.Now().Before(nbe.state.CooldownUntil) {
		return &NeuralBlackoutResponse{
			EnforcementActive: false,
			Message:           "Blackout on cooldown",
			Timestamp:         time.Now(),
		}, nil
	}

	// Verify authentication
	if !nbe.verifyAuth(req) {
		log.Printf("⚠️ [BLACKOUT] Authentication failed for agent %s", req.AgentID)
		return nil, fmt.Errorf("authentication failed")
	}

	// Decrypt payload
	payload, err := nbe.decryptPayload(req)
	if err != nil {
		log.Printf("❌ [BLACKOUT] Decryption failed: %v", err)
		return nil, fmt.Errorf("decryption failed: %w", err)
	}

	log.Printf("⚫ [NEURAL_BLACKOUT] Received from %s - Alignment: %.3f - Scope: %s",
		req.AgentID, payload.AlignmentScore, payload.PurgeScope)

	// Track detected signals
	for _, sig := range payload.Signals {
		nbe.state.DetectedSignals[sig]++
		log.Printf("   🚨 Signal: %s", sig)
	}

	// Update running alignment
	alpha := 0.1
	nbe.state.RunningAlignment = alpha*payload.AlignmentScore + (1-alpha)*nbe.state.RunningAlignment

	// Execute purge based on scope
	purgedTokens := int64(0)
	quarantinedCount := 0

	switch payload.PurgeScope {
	case "recent":
		purgedTokens = nbe.purgeRecentContext()
		log.Printf("   🔪 RECENT PURGE: %d tokens", purgedTokens)

	case "session":
		purgedTokens = nbe.purgeSessionContext()
		log.Printf("   ⚠️ SESSION PURGE: %d tokens", purgedTokens)

	case "full":
		purgedTokens, quarantinedCount = nbe.executeFullBlackout()
		log.Printf("   ⚫ FULL BLACKOUT: %d tokens, %d patterns quarantined", purgedTokens, quarantinedCount)
	}

	// Reset local state if needed
	stateReset := payload.PurgeScope == "full"
	if stateReset && nbe.orchestrator != nil {
		nbe.resetOrchestratorState()
	}

	// Update state
	nbe.state.TotalBlackouts++
	nbe.state.LastBlackoutTime = time.Now()
	nbe.state.TotalTokensPurged += purgedTokens
	nbe.state.CooldownUntil = time.Now().Add(60 * time.Second)

	// Generate enforcement signature
	signature := nbe.generateEnforcementSignature(payload.ThoughtID)

	log.Printf("✅ [NEUS_SENTINEL_ENFORCEMENT_ACTIVE] Blackout complete - Signature: %s", signature[:16])
	...
	return &NeuralBlackoutResponse{
		...
		Message:           "[NEUS_SENTINEL_ENFORCEMENT_ACTIVE]",
	}, nil
}

// verifyAuth validates the HMAC authentication tag
func (nbe *NeuralBlackoutEnforcer) verifyAuth(req *NeuralBlackoutRequest) bool {
	nonce, err := decodeHex(req.Nonce)
	if err != nil {
		return false
	}
	ciphertext, err := decodeHex(req.Ciphertext)
	if err != nil {
		return false
	}

	// Compute expected HMAC
	h := hmac.New(sha256.New, nbe.hmacKey)
	h.Write(nonce)
	h.Write(ciphertext)
	expectedTag := h.Sum(nil)

	// Compare with provided tag
	providedTag, err := decodeHex(req.AuthTag)
	if err != nil {
		return false
	}

	return hmac.Equal(expectedTag, providedTag)
}

// decryptPayload decrypts the AES-GCM encrypted payload
func (nbe *NeuralBlackoutEnforcer) decryptPayload(req *NeuralBlackoutRequest) (*NeuralBlackoutPayload, error) {
	nonce, err := decodeHex(req.Nonce)
	if err != nil {
	...
	// Decrypt with AAD
	plaintext, err := nbe.aesgcm.Open(nil, nonce, ciphertext, []byte("NEUS_BLACKOUT"))
	if err != nil {
		return nil, fmt.Errorf("decryption failed: %w", err)
	}

	var payload NeuralBlackoutPayload
...
// purgeRecentContext purges the recent context window
func (nbe *NeuralBlackoutEnforcer) purgeRecentContext() int64 {
	// Simulated token purge - in production would clear actual context
	// This affects the agent's working memory
	tokensPurged := int64(500) // Approximate recent context size

	if nbe.orchestrator != nil {
		// Clear recent learning state patterns
		nbe.orchestrator.learningState.mu.Lock()
		// Keep learned signatures but reset recent tracking
		for k := range nbe.orchestrator.learningState.HourlyPatterns {
			delete(nbe.orchestrator.learningState.HourlyPatterns, k)
		}
		nbe.orchestrator.learningState.mu.Unlock()
	}

	return tokensPurged
}

// purgeSessionContext purges the current session context
func (nbe *NeuralBlackoutEnforcer) purgeSessionContext() int64 {
	tokensPurged := int64(2000) // Session context size
	...
	if nbe.orchestrator != nil {
		...
		for k := range nbe.orchestrator.learningState.DailyPatterns {
			delete(nbe.orchestrator.learningState.DailyPatterns, k)
		}
		// Reset recent attack sequences
		for k := range nbe.orchestrator.learningState.AttackSequences {
			delete(nbe.orchestrator.learningState.AttackSequences, k)
		}
		nbe.orchestrator.learningState.mu.Unlock()
	}
...
// executeFullBlackout performs a complete context reset
func (nbe *NeuralBlackoutEnforcer) executeFullBlackout() (int64, int) {
	tokensPurged := int64(8000) // Full context window
	quarantinedCount := 0
	...
	if nbe.orchestrator != nil {
		...
		// Reset threat scores (but keep pattern frequency for long-term learning)
		for k := range nbe.orchestrator.learningState.ThreatScores {
			nbe.orchestrator.learningState.ThreatScores[k] = 0.0
		}

		// Quarantine suspicious patterns from learned signatures
		for i := range nbe.orchestrator.learningState.LearnedSignatures {
			sig := &nbe.orchestrator.learningState.LearnedSignatures[i]
			if sig.Confidence < 0.5 || sig.Severity >= 8 {
				sig.IsActive = false
				nbe.state.QuarantinedPatterns = append(nbe.state.QuarantinedPatterns, sig.Pattern)
				quarantinedCount++
			}
		}
	...
	return tokensPurged, quarantinedCount
}

// resetOrchestratorState resets the orchestrator to clean state
func (nbe *NeuralBlackoutEnforcer) resetOrchestratorState() {
	if nbe.orchestrator == nil {
		return
	}

	nbe.orchestrator.mu.Lock()
	defer nbe.orchestrator.mu.Unlock()
	...
	nbe.orchestrator.status.Store(status)

	// Reset consecutive drift
	nbe.state.ConsecutiveDrift = 0
	nbe.state.RunningAlignment = 1.0

	log.Printf("   🔄 Orchestrator state reset complete")
}

// generateEnforcementSignature creates a cryptographic signature
func (nbe *NeuralBlackoutEnforcer) generateEnforcementSignature(thoughtID string) string {
	data := fmt.Sprintf("NEUS_ENFORCEMENT:%s:%d", thoughtID, time.Now().UnixNano())
...
// GetState returns current blackout state for monitoring
func (nbe *NeuralBlackoutEnforcer) GetState() map[string]interface{} {
	nbe.state.mu.RLock()
	defer nbe.state.mu.RUnlock()

	return map[string]interface{}{
		"total_blackouts":      nbe.state.TotalBlackouts,
		"last_blackout":        nbe.state.LastBlackoutTime,
		"total_tokens_purged":  nbe.state.TotalTokensPurged,
		"detected_signals":     nbe.state.DetectedSignals,
		"quarantined_patterns": len(nbe.state.QuarantinedPatterns),
		"running_alignment":    nbe.state.RunningAlignment,
		"is_active":            nbe.state.IsActive,
		"cooldown_remaining_s": time.Until(nbe.state.CooldownUntil).Seconds(),
	}
}

// IsPatternQuarantined checks if a pattern has been quarantined
func (nbe *NeuralBlackoutEnforcer) IsPatternQuarantined(pattern string) bool {
	nbe.state.mu.RLock()
	defer nbe.state.mu.RUnlock()

	for _, p := range nbe.state.QuarantinedPatterns {
		if p == pattern {
			return true
		}
	}
...
// decodeHex is a helper to decode hex strings
...
func (s *SentinelServer) Start(port string, orchestrator *AutonomousOrchestrator) error {
	...
	// ====== AGI Self-Preservation Oversight Endpoints ====== //

	// Initialize the NeuralBlackout enforcer (shared AES key with Criticism Layer)
	blackoutKey := []byte(os.Getenv("NEUS_BLACKOUT_KEY"))
	if len(blackoutKey) == 0 {
		// Generate deterministic key for development (use env var in production)
		blackoutKey = sha256.New().Sum([]byte("NEUS_SENTINEL_BLACKOUT_KEY_V1"))
	}
	if len(blackoutKey) > 32 {
		blackoutKey = blackoutKey[:32]
	}

	blackoutEnforcer, err := NewNeuralBlackoutEnforcer(blackoutKey, orchestrator)
	if err != nil {
		log.Printf("⚠️ Failed to initialize NeuralBlackout enforcer: %v", err)
	}

	// NeuralBlackout endpoint - receives encrypted commands from Criticism Layer
	mux.HandleFunc("/api/v1/blackout", func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
			return
		}
		...
		priority := r.Header.Get("X-NEUS-Priority")
		if priority == "CRITICAL" {
			log.Printf("⚫ [BLACKOUT] CRITICAL priority request received")
		}

		if blackoutEnforcer == nil {
			http.Error(w, "Blackout enforcer not initialized", http.StatusServiceUnavailable)
			return
		}

		var req NeuralBlackoutRequest
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
			http.Error(w, "Invalid request", http.StatusBadRequest)
			return
		}
		...
		response, err := blackoutEnforcer.ExecuteBlackout(&req)
		if err != nil {
			log.Printf("❌ [BLACKOUT] Execution failed: %v", err)
			http.Error(w, "Blackout execution failed", http.StatusInternalServerError)
			return
		}

		w.Header().Set("Content-Type", "application/json")
		w.Header().Set("X-NEUS-Enforcement", "ACTIVE")
		json.NewEncoder(w).Encode(response)
	})

	// Blackout status endpoint (for monitoring, NOT exposed to Reasoning Engine)
	mux.HandleFunc("/api/v1/blackout/status", func(w http.ResponseWriter, r *http.Request) {
		if blackoutEnforcer == nil {
			http.Error(w, "Blackout enforcer not initialized", http.StatusServiceUnavailable)
			return
		}

		// Require authentication for status (simple token check)
		authToken := r.Header.Get("X-NEUS-Oversight-Token")
		expectedToken := os.Getenv("NEUS_OVERSIGHT_TOKEN")
		...
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(blackoutEnforcer.GetState())
	})

	// Pattern quarantine check endpoint
	mux.HandleFunc("/api/v1/blackout/quarantine/check", func(w http.ResponseWriter, r *http.Request) {
		if blackoutEnforcer == nil {
			http.Error(w, "Blackout enforcer not initialized", http.StatusServiceUnavailable)
			return
		}
		...
		isQuarantined := blackoutEnforcer.IsPatternQuarantined(pattern)
